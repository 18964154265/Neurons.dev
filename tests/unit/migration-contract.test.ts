...
const cancelMigration = readFileSync(
  join(process.cwd(), "supabase/migrations/202608290003_confirm_run_cancel.sql"),
  "utf8",
);
...
describe("initial database migration", () => {
  ...
  it("confirms workflow cancellation atomically", () => {
    ...
    expect(cancelMigration).toContain("set active_run_id = null, status = 'stopped'");
    expect(cancelMigration).toContain("to authenticated");
    expect(cancelMigration).toContain("function public.fail_run_workflow_start");
