  ...
  try {
    ...
    const [definitionsResult, assignmentsResult] = await Promise.all([
      supabase
        ...
        .select("agent_key,definition_version,display_name,description,avatar_path,tool_labels")
        .eq("enabled", true)
        .order("display_name"),
      supabase
    ...
    if (definitionsResult.error || assignmentsResult.error) {
      throw new ApiError("AGENT_LIST_FAILED", 500, "Agent 列表加载失败。", true);
    }
    const assignments = new Map(
      (assignmentsResult.data ?? []).map((assignment) => [assignment.agent_key, assignment]),
    );
